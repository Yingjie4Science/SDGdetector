...
#     ## to exclude these keywords
#     which_sdg_term <- gsub(pattern = "\\.\\+", replacement = terms_replacement, which_sdg_term)
#   } else {
...
func_to_exclude_terms <- function(which_sdg_term, terms_to_exclude) {
  ...
  pat_exclude <- paste0("^(?!.*(?:", paste(terms_to_exclude, collapse = "|"), ")).*")
...
#' @param third_AND_string similar to word_ls1 or word_ls2, it is a string that includes
...
lookaround_nearby_n <- function(word_ls1,
                                word_ls2,
                                n,
                                exclude = "",
                                third_AND_string = "") {
  ...
  if( nchar(pat_exclude)<1 ) {
    ...
    ## remove the ".*^" between `pat_exclude` and `pat_and`
    pat_exclude_and <- gsub('\\.\\*\\^', '', pat_exclude_and);
    # pat_exclude_and
  }
...
### child_ls ----
child_ls = paste(
  "child\\S*|\\bteen.?\\b|teenage\\S*|juvenile\\S*|youth|young|\\bminor.?\\b|under 5|under five|underfive|before fifth|adolescen\\S*",
...
### company_ls ----
company_ls <-
  "\\bcompany\\b|\\bcompanies|\\bfirm.?\\b|\\bcorporate\\b|\\bcorporation\\S*|business|enterprise\\S*"
...
### emission_ls ----
emission_ls = paste(
  "\\bemission.?|\\bCO2\\b|carbon|green.?house|\\bGHG\\b|G\\.H\\.G\\.|Halocarbons",
...
### equal_ls ----
equal_ls = paste(
  "equal\\S*|\\bequitab\\S*|\\bequalit\\S*|\\bun.?bias\\S*|unprejudic\\S*",
...
### increase_ls ----
increase_ls = paste(
  "accelerat\\S*|ascend\\S*|\\badvanc\\S*|augment\\S*|\\bamplif\\S*|magnif\\S*|burgeon\\S*",
...
renewable_ls_x = c(
  paste(
    "\\badvanc\\S*|alcohol|modern|\\bclean\\b|\\brenewable|alternative\\b|ethanol|biorefinery|\\bwind\\b|solar|biomass|nuclear",
...
renewable_ls_y = paste(
  "(\\brenewables|biofuel|bioenergy|biodiesel|bio.?gas|bioethanol|hydropower",
  "hydroelectric|electrofuel|\\be.?fuel|photovoltaic|wind farm\\S*|offshore wind|wind turbine\\S*|solar panel\\S*)",
  sep = "|"
)
renewable_ls_x <-
  func_AND_vector(renewable_ls_x) ## not suitable to use `func_AND_plus` here
...
temp    <-
  paste(
    "malnutrition.?|malnourish|undernourish|undernutrition|under nourished|stunting",
...
SDG2_2_x = c(
  ...
  paste(
    child_ls,
    "infant\\S*|neonate\\S*|newborn|baby|babies|toddler|older",
    sep = '|'
  )
)
SDG2_2_y = c(temp,
...
SDG2_a_x = c(
  paste(
    support_ls,
    "\\badvanc\\S*|cooperat\\S*|collaborat\\S*|joint effort.?|\\bflow\\b|\\bflows\\b",
    sep = '|'
  ),
  "rural|agricult\\S*",
...
SDG2_b_x = c(
  ...
  paste(
    "\\bsubsidy|\\bsubsidies|\\bsubsidiz|\\bsubsidis|restrict\\S*|distort\\S*|allowance\\S*",
    support_ls,
    sep = '|'
  )
)
...
SDG3_1 = c(
  paste(
    "matern\\S*|antenatal\\S*|birth|gestational|parturient",
    func_AND_plus(c("post", "natal|partum")),
    sep = "|"
  ),
...
SDG3_2 = c(
  paste(
    child_ls,
    "newborn\\S*|foetal|premature|preterm|pediatric\\S*|perinatal|infant.?|neonat\\S*",
    sep = '|'
  ),
...
SDG3_4_x1 = c(
  paste(
    ...
    "diabet\\S*|copd|coad|asthma\\S*|diarrhea|diarrhoea|dysentery|\\bobes\\S*|suicid\\S*|stroke",
    sep = '|'
  ),
  death_ls,
  reduce_ls
...
SDG3_5 = c(
  "substance|drug|alcohol|drinking|drunk|ethanol|liquor|liqueur|booze|\\bwine|\\bbeer.?|Narcotic",
...
SDG3_6_x = c(
  paste(
    "\\broad.?\\b|\\broadway|\\blane\\S*|avenue|street|\\bdrive\\b|\\bcar\\b|automobile|\\bmotor|vehicle.?",
...
SDG3_8_y <-
  func_to_exclude_terms(which_sdg_term = SDG3_8_y, terms_to_exclude = financial_exclude_ls)


SDG3_9_x = c(
  "hazard\\S*|unsafe|unintentional|inadequate|contamin\\S*|pollut\\S*",
...
SDG3_a = c(
  "tobacco|nicotine|cigar\\S*|\\bvap\\S*|smok\\S*" ,
  paste(
    "control|\\bregulat\\S*|administer\\S*|\\bban\\b|\\bbans\\b|\\bbanned\\b|govern\\S*|manag\\S*|cessation\\S*|\\bquit\\S*",
    reduce_ls,
    sep = "|"
  )
)
...
SDG3_a   <-
  gsub(
    "\\.\\+",
    "(?!.*government)(?!.*Governance)(?!.*Management)(?!.*manager)",
    SDG3_a
  ) ## to exclude these
...
SDG3_b_x = c(
  paste(
    "vaccin\\S*|medicin\\S*|medication\\S*|antibiotic",
    "health facilit\\S*|health sector\\S*",
    sep = '|'
  ),
...
SDG3_b_y = c(
  paste(
    "vaccin\\S*|medicin\\S*|medication\\S*|antibiotic|pharmac\\S*",
    "health facilit\\S*",
    sep = '|'
  ),
...
SDG3_b_x <-
  gsub(
    "\\.\\+",
    "(?!.*development expense)(?!.*investigat\\S*)(?!.*Invest Ltd)",
    SDG3_b_x
  ) ## to exclude these
SDG3_b_y <- func_to_exclude_terms(
  which_sdg_term = SDG3_b_y,
  terms_to_exclude =  c(
    "available data",
    "extension",
    "market access",
...
SDG3_c_x = c(
  "\\bhealth\\b|well.?being",
  paste(
    "financ\\S*|\\bfund.?\\b|\\bfunding\\b|\\bassistance.?|\\baid.? to",
...
SDG4_2_x = c(
  paste(child_ls, "infant|toddler", sep = "|"),
  "\\bcare.?\\b|pre.?primary educat\\S*|health|well.?being|psycho\\S*"
)
SDG4_2_x <- func_AND_plus(SDG4_2_x)
SDG4_2_x <-
  gsub("\\.\\+",
       "(?!.*(?:young|Children.?s Centre|Children.?s Center))",
       SDG4_2_x) ## to exclude these
...
temp   <- SDG4_2_z
SDG4_2_z <-
  lookaround_nearby_n(word_ls1 = temp[1],
                      word_ls2 = temp[2],
                      n = 4)
...
temp     <-
  paste(
    "access|availab\\S*|equitab\\S*|equal\\S*|\\bfree\\b|affordab\\S*|quality|particip\\S*",
    "\\bjoin\\b|\\bjoins|\\bjoined|\\bjoining|attend\\S*|lifelong|adult\\S*|non.?traditional",
    sep = "|"
  )
SDG4_3_x <- c(
...
SDG4_3_y <-
  c("\\bGED\\b|G\\.E\\.D\\.|General Educational Development",
    temp)
SDG4_3_x <- func_AND_plus(SDG4_3_x)
SDG4_3_y <- func_AND_plus(SDG4_3_y)
SDG4_3_x <-
  func_to_exclude_terms(
    which_sdg_term = SDG4_3_x,
    terms_to_exclude = c('Access Bank', 'Information is available', 'job fair')
  ) ## to exclude these
SDG4_3_y <-
  func_to_exclude_terms(
    which_sdg_term = SDG4_3_y,
    terms_to_exclude = c('Access Bank', 'Information is available', 'job fair')
  ) ## to exclude these


SDG4_4_x = c(
  "technical|vocational|information and communications|\\bICT\\b",
...
SDG4_7_x = c(
  "\\bknowledge|skill\\S*|educat\\S*|\\btraining\\S*|curricul\\S*|teach\\S*|student assessment.?",
...
SDG4_7_y = c(
  "global citizenship|international|Universal|\\bpolicy|\\bpolicies|Teacher.?\\b|trainer.?\\b|educator.?\\b",
  "education"
)
...
SDG4_b = c(
  paste(
    "scholarship.?|fellowship.?|official development assistance",
    "financ\\S*|\\bfund.?\\b|\\bfunding\\b|\\baid.? to|government aid|state aid|invest\\S*|\\bgrant.?\\b|official flow.?",
    sep = "|"
  ),
  paste(
    "knowledge|skill\\S*|educat\\S*|vocational training.?|information technology|communications technology",
    "information and communications technology",
    "\\btechnical\\b|engineer\\S*|scien\\S*|mathemat\\S*|technolo\\S*|\\bSTEM\\b|S\\.T\\.E\\.M\\.",
    sep = "|"
  ),
  developing_country_ls
)
...
SDG4_b <-
  gsub("\\.\\+", "(?!.*Corporate Governance)", SDG4_b) ## to exclude these



SDG4_c = c(
  paste(
    'teacher\\S*|educator\\S*|faculty|instructor\\S*|lecturer\\S*|professor\\S*|supervisor\\S*",
                 "mentor.?\\b|tutor\\S*|trainer\\S*|adviser\\S*|advisor\\S*|\\bcoach\\b|\\bcoaches\\b|caregiver\\S*',
    sep = "|"
  ),
...
SDG5_1_x = c(
  paste(
    "equality|\\bparity|justice|segregat\\S*|\\banti\\S*|hostile|marginali\\S*",
    discrimination_ls,
    sep = "|"
  ),
...
SDG5_2_x = c(
  "violence|assault\\S*|attack\\S*|aggression|offens\\S*|\\babuse|traffick\\S*|exploit\\S*|\\bforc\\S*|misconduct\\S*",
...
SDG5_3_x = c(
  paste(
    child_ls,
    "\\bharm.?\\b|\\bharmed\\b|\\bearly|forced|before age 15|before age 18",
    sep = "|"
  ),
  "marr\\S*"
)
...
SDG5_4 = c(
  "unpaid|shared responsibility|allocation|equity|equitab\\S*|fairness",
  paste(
    "\\bcare.?\\b|domestic work|household work|family work\\S*",
    female_ls,
    sep = "|"
  )
)
...
SDG5_5 = c(
  paste(
    "leadership|leader\\S*|\\bseat\\S*|decision.?making|national parliament\\S*|government.?|autonomy",
    "managerial position.?|political participation.?|politician|manager.?|in middle management|in senior management",
    "in executive position.?|under.?representation|entrepreneurship|\\bpower|glass ceiling",
    sep = "|"
  ),
  female_ls
)
...
SDG5_6_x = c(
  "access|availab|guarantee\\S*|\\bright.?|decision.?|decide.? on|autonomy|\\brights|\\blaw\\S*|regulation.?|information|educat\\S*",
...
SDG5_6_z = c("decision.?|\\breproduct\\S*",
             ...
             female_ls)
SDG5_6_w = paste(
  "parent.? leave|Parental leave|paternity leave|family leave|adoption leave",
...
SDG5_a_x = c(
  "access|availab\\S*|\\bright.?\\b|\\bcontrol|\\breform\\S*|equal|\\blegal\\S*|\\blaw\\S*",
  "resource.?|\\bfund|\\bland\\b|propert\\S*|financial service.?|finance|owner\\S*|budget.?|\\bquota\\b",
  paste(female_ls, "\\bgender", sep = "|")
)
SDG5_a_y = c(
  paste(
    "ownership|inheritance right.?|right.? of inheritance|right.? to inherit\\S*",
    "Mandatory quota.?|land tenure|land management|rights.?bearer\\S*",
    sep = "|"
  ),
  paste(female_ls, "\\bgender", sep = "|")
)
...
SDG5_b = c(
  "technolog\\S*|information|communications|mobile.?phone.?|telephone.?|smart.?phone.?|Cell.?phone.?|\\bICT\\b",
  "\\bempower\\S*|promot\\S*",
  female_ls
)
...
SDG5_c = c(
  paste(
    "\\bpolicy|\\bpolicies|program.?|legislation.?|\\bregulation.?|\\blaw\\b|\\blaws",
...
SDG6_1 = c(
  "access|availab\\S*|afford\\S*|safe|secure|\\bclean|manag\\S*|equitab\\S*",
  "drinking water|drinkable water"
)

SDG6_2_x = c(
  "access|availab\\S*|safe|manag\\S*|adequate\\S*|equitab\\S*|equal|facilit\\S*|service.?",
  paste(
    "sanita\\S*|hygien\\S*|soap|cleanliness",
    "hand wash\\S*|wash.*hand\\S*",
    sep = '|'
  )
)
SDG6_2_y = c(
  "access|availab\\S*|safe|\\bopen|hygien\\S*|adequate|equal|health|\\bclean|manag\\S*",
...
SDG6_3_z = c('\\bwater.?\\b|aquatic',
             'quality',
             paste('good', increase_ls, sep = "|"))
SDG6_3_w = c(
  "dump\\S*|releas\\S*|recycl\\S*|\\bre.?us\\S*|untreated|\\btreated\\b|sustainab\\S*",
...
SDG6_4_x = c(
  "\\bwater.?\\b|freshwater|groundwater",
...
SDG6_4_y = c(
  "\\bwater.?\\b|freshwater|groundwater",
  paste(
    "footprint.?|withdraw\\S*|consumption\\S*|consumed|\\busage\\S*|\\bgreen\\b|blue\\b|grey",
    "scarcity|scarce|scant|shortage|\\bstress|dearth|deficit|\\black|shortfall",
    "insufficien\\S*|inadequa\\S*|deficienc\\S*",
    sep = "|"
  ),
  paste(
    reduce_ls,
    'address\\S*|tackl\\S*|response to|cope.?with|coping with|deal.?with|dealing with|handl\\S*',
    sep = "|"
  )
)
...
SDG6_5 = c(
  ...
  paste(
    "manag\\S*|transboundary|transnational|international|interbasin|cooperat\\S*|collaborat\\S*|joint effort.?|co.?operation|coping",
    policy_ls,
    sep = "|"
  )
)
...
SDG6_6 = c(
  "\\bwater.?\\b|mountain\\S*|forest|wetland|river|aquifer|lake.?",
  "eco.?system.?",
...
SDG6_a_x = c(
  "\\bwater.?\\b|sanita\\S*",
  paste(
    "development assistance|harvesting|desalina|\\befficien\\S*|waste\\S*|\\btreatment.?",
...
SDG6_b =  c(
  "\\bwater.?\\b|sanita\\S*|hygiene|wastewater treatment",
  paste(
    "\\blocal communit\\S*|local administrat\\S*",
...
SDG7_1_x = c(
  paste('energy|\\bfuel|\\bpower\\b', "electricity", sep = "|"),
...
SDG7_2_y = paste("energy transition.?",
...
SDG7_2_z = c(
  paste(
    "fossil.?fuel.?|fossil.?gas|\\bcoal\\b|\\bcoals\\b|petrol|natural gas|methane|crude oil.?",
...
temp <- SDG7_2_z
ex <-
  strsplit(x = "year end|coal price\\S*|coal cost\\S*", split = "\\|")
ex <- unlist(ex)
SDG7_2_z <-
  lookaround_nearby_n(
    word_ls1 = temp[1],
...
SDG7_3_x = c(
  "energy|\\belectric\\S*|\\bfuel.?|\\bpower\\b|\\butilit\\S*",
  "\\befficien\\S*|\\bintens\\S*|optimization tech\\S*|productivity|pollut\\S*|footprint.?|\\bleak.?\\b|\\bleakage.?|\\bper GDP|\\bper capita"
)
SDG7_3_y = c(
  "energy consumption|energy use\\S*|energy usage|energy inefficiency|energy.?intensive|energy burden|energy waste",
  reduce_ls
)
...
SDG7_a_x <-
  func_to_exclude_terms(which_sdg_term = SDG7_a_x, terms_to_exclude = ex)


SDG7_a_y = c(
  'energy|\\belectricit\\S*|\\bfuel.?|electric vehicle.?|hybrid vehicle.?',
...
SDG7_general <-
  gsub("\\.\\+", "(?!.*Renewables segment\\S*)", SDG7_general) ## to exclude these keywords
...
SDG8_1 = c(
  economic_ls,
  "per capita",
  paste("Sustain\\S*|develop\\S*|prosper\\S*", increase_ls, sep = "|")
) ## ,"nation|countr|state"
...
SDG8_2 <-
  func_to_exclude_terms(which_sdg_term = SDG8_2, terms_to_exclude = financial_exclude_ls)
...
SDG8_3_z1 = c(
  "productive activit\\S*|entrepreneurship\\S*",
...
SDG8_3_z2 = c(
  "micro.?|small.?|medium.?|starting|start.?up.?|kickstart|inclusiv\\S*",
...
SDG8_5_x1 = c(
  paste(
    "\\bfull\\b|\\bdecent\\b|\\bseemly\\b|respectable|productiv\\S*",
...
SDG8_5_x2 = c(
  paste(discrimination_ls, "informal\\b|precarious", sep = "|"),
...
SDG8_5_x4 = c(
  ...
  "opportunit\\S*",
  paste(
    "people|person|\\bemployee\\S*|\\bstaff\\S*|\\bworker.?|\\bwork\\b|\\bworking|\\bgender|\\bsex|ethni\\S*|\\brace|racial",
    "\\breligion",
    disability_ls,
    "disadvantaged",
    female_ls,
...
SDG8_5_x5 = c(
  "\\bwage|\\bpay.?\\b|\\bpaying|\\bpaid|\\bsalary|\\bsalaries|\\bearning|\\bstipend",
  paste(
    "\\bgap\\b|difference|discrepan\\S*|dissimilarit\\S*|disproport\\S*|divergen\\S*|imbalance\\S*",
    discrimination_ls,
    sep = "|"
  ),
  reduce_ls
...
SDG8_5_x6 = c(
  "Share of|proportion|\\bpercent\\S*|discrimination",
  paste(
    disability_ls,
...
SDG8_5_x7 = c(
  ...
  paste(
    "\\baddress\\S*|tackl\\S*|response to|cope.?with|coping with|deal.?with|dealing with|handl\\S*",
...
SDG8_5_x8 = c(
  "employment.?|\\bhiring|\\bemployee.?|\\bjob\\S*|\\bworker.?|\\blabor\\b|\\blabour\\b|coverage",
  disability_ls
)
...
SDG8_6 = c(
  "youth|young people|\\bteen.?\\b|teenage\\S*",
  "\\bin\\b|\\bwith\\b",
...
SDG8_8_x4 <- paste("zero incident.?",
                   "retirement fund\\S*|secure retirement.?",
                   sep = "|")
...
SDG8_8_x5 = c(
  paste(
    death_ls,
    "injur\\S*|\\bharm.?\\b|\\bharmed\\b|danger\\S*|\\bincident\\S*",
    sep = "|"
  ),
...
SDG8_8_x6 <-
  c(
    paste(
      "ensure|prioritiz\\S*|\\bstimulat\\S*|management\\S*",
      increase_ls,
      sep = "|"
    ),
...
SDG8_9_x = c(
  "touris\\S*",
  paste(
    "sustainab\\S*|community.?based|\\bemployment.?",
    func_AND_plus(c('\\bcreat\\S*', '\\bjob.?')),
    sep = "|"
  )
)
...
SDG8_9_y <-
  func_AND_plus(c(
    "\\btour\\S*",
    "\\bpolicy|\\bpolicies|\\bGDP\\b|Gross Domestic Product"
  ))
SDG8_9_z <- 'ecotourism'
...
SDG8_10 <-
  func_to_exclude_terms(
    which_sdg_term = SDG8_10,
    terms_to_exclude = c(financial_exclude_ls, "Access Bank")
  )
...
SDG9_1   = c(
  "quality|reliab\\S*|sustainable|resilien\\S*|afford\\S*|equitab\\S*|inclus\\S*",
  infrastructure_ls
)
temp <- SDG9_1
SDG9_1 <-
...
SDG9_3_x = c(
  "\\bindustr\\S*|enterprise\\S*|entrepreneur\\S*",
...
SDG9_3_y = c(
  "small.?scale industr\\S*|micro.?enterprise\\S*|small.?sized business",
  "Share of|proportion\\S*|\\bpercent\\S*"
)
SDG9_3_z = c(
  "\\bsmall|\\bmicro.?\\b|starting|start.?up.?|kickstart",
  paste("entrepren\\S*", company_ls, sep = "|"),
  paste(
    "Share of|proportion\\S*|\\bpercent\\S*|financial service.?|affordable credit.?",
...
SDG9_3_x <-
  gsub("\\.\\+", "(?!.*Access Bank)", SDG9_3_x) ## to exclude
...
SDG9_4_w = c(
  "\\bclean|environmentally sound|\\bgreen\\b|\\bgreener\\b",
...
SDG9_5_x = c(
  "technolog\\S*|innov\\S*|\\binvention|research\\S*",
  paste(
    increase_ls,
  ...
  paste(
    "\\bindustr\\S*|manufactur\\S*|business\\S*|\\boperation.?|\\bspending.?|\\bGDP\\b|gross domestic product",
...
temp <- SDG9_5_x
w_ex <-
  "incremental cost|Technology Development Zone|COMBINED MANAGEMENT REPORT"
...
SDG9_5_y = c(
  paste(
    "scientific research|Researcher.?|scientist.?|\\bscholar.?\\b|specialist\\S*|\\bexpert.?\\b",
    "\\bsavant.?\\b|academician|professor.?|Research and development",
    "\\bR&D\\b|\\bR & D\\b|\\bR-D\\b|\\bRTD\\b|\\bR\\+D\\b",
    sep = "|"
  ),
...
SDG9_b_x <- c(
  "technolog\\S*|innovation.?|research|medium.?tech\\S*|high.?tech\\S*|\\bR&D\\b|\\bR & D\\b|\\bR-D\\b|industrial diversification",
  paste(increase_ls, support_ls, sep = "|"),
  paste(
    "operational efficiency|domestic",
    developing_country_ls,
    sep = "|"
  )
)
ex_ls <-
  "Award.?|Financial Report.?|Annual Report.?|University of Science and Technology" %>%
  strsplit(., split = "\\|") %>%
  unlist()
SDG9_b_x <- func_AND_plus(SDG9_b_x)
SDG9_b_x <-
  func_to_exclude_terms(which_sdg_term = SDG9_b_x, terms_to_exclude = ex_ls)


SDG9_b_y <- c(
  "medium.?tech\\S*|high.?tech\\S*",
  "value added"
)
...
ex_ls <-
  "Access Bank|Access Line.?|inappropriate access|broadband accesses retail|Internet of Thing" %>%
  strsplit(., split = "\\|") %>%
  unlist()
temp <- SDG9_c_x
SDG9_c_x <-
...
SDG10_1 <- c(
  "income|livelihood|expenditure|earning|\\bpay\\b|\\bwage",
...
SDG10_2_x <- c(
  "inclusi\\S*",
  paste(
    "empower\\S*|\\breform\\S*|Prioritiz\\S*|\\bstimulat\\S*|strengthen\\S*|support\\S*|\\bcommitment\\S*|provid\\S*",
    increase_ls,
    sep = "|"
  ),
  paste(
    "social|economic|socio.?economic|political|\\bage\\b|\\bsex|\\bgender",
    female_ls,
    LGBT_ls,
    disability_ls,
...
SDG10_2_x <-
  gsub("\\^", "^(?!.*(?:financial inclusion.?))", SDG10_2_x) ## exclude "financial inclusion"
...
SDG10_3_x = c(
  paste(
    discrimination_ls,
    "harass\\S*|homophobia\\S*|racism|sexism",
    sep = "|"
  ),
  reduce_ls
)
SDG10_3_y = c(
  "equal opportunit\\S*|equal treatment.?|\\bequitab\\S*|\\bequality|\\bun.?bias\\S*|unprejudic\\S*|impartial\\S*|fairness",
  paste(
    "\\bensure|\\bassur\\S*|guarantee\\b|promot\\S*|empower\\S*|support\\S*|\\bstimulat\\S*|strengthen\\S*",
    increase_ls,
    sep = "|"
  )
)
...
SDG10_5 = c(
  "\\bregulat\\S*|monitor\\S*|administer\\S*|govern.?\\b|governed|governing|\\bsound\\b",
  "financial market.?|financial institution.?"
)
ex_ls   <-
  "regulatory|regulator.?|General government|Corporate Governance|Financial Institutions Act|Pension Fund\\S*" %>%
  strsplit(., split = "\\|") %>% unlist()
SDG10_5 <- func_AND_plus(SDG10_5)
SDG10_5 <-
  func_to_exclude_terms(which_sdg_term = SDG10_5, terms_to_exclude = ex_ls) ## to exclude



SDG10_6_x = c(
  "representation.?|\\bvoic\\S*|\\bright.?|\\bvot\\S*|effective|credib\\S*|accountabl\\S*|legitimate|justif\\S*|lawful",
...
SDG10_6_x <-
  func_to_exclude_terms(which_sdg_term = SDG10_6_x, terms_to_exclude = financial_exclude_ls)
...
SDG10_a = c(
  paste(reduce_ls, "\\bspecial|differential|\\bfree\\b", sep = '|'),
...
SDG10_b_x = c(
  "financ\\S*|\\bfunding|\\bassistance|budget.?|\\baid.? to|government aid.?\\b|state aid.?\\b|invest\\S*|donor|donat\\S*|official flow.?",
  "develop\\S*|resource.?|capabilit\\S*|national plan\\S*|program\\S*",
  developing_country_ls
)
SDG10_b_y = c(
  "north-south divide\\S*|financial flow.?|resource flow.?|foreign direct investment.?|\\bFDI\\b|\\bODA\\b",
  developing_country_ls
)
...
SDG10_general = c(
  "\\bequal\\S*|inequalit\\S*|unequal\\S*",
...
SDG11_1_x = c(
  "access|availab\\S*|affordab\\S*|adequate|formal|safe|insufficient|deficient|\\black|scarce|scant|unsatisfactory",
  paste(
    "\\bhouse\\b|\\bhouses\\b|\\bhousing\\b|\\bhome\\b|apartment.?|condo|dwelling\\S*|residence|accommodation\\S*|\\bshelter",
    "basic service.?|sanitation.?|hygiene|electricity|slum\\b|slums\\b|slummy|Shanty|\\bliving",
    sep = "|"
  )
)
SDG11_1_y = c(
  paste(urban_ls, reduce_ls, "upgrad\\S*|enhanc\\S*", sep = "|"),
...
SDG11_4_x = c(
  paste(
    "protect\\S*|safeguard\\S*|preserv\\S*|conserv\\S*|assure|retain\\S*",
    "maintenance|preventive method\\S*|preventive measure.?",
    support_ls,
    sep = "|"
  ),
  paste(
    "heritage|archaeological site.?|geological site.?|monumental sculpture\\S*|monumental painting\\S*",
...
SDG11_4_y = c(
  ...
  paste(
    "Cultural\\S*|symbolic\\S*|historic\\S*|artistic\\S*|aesthetic\\S*|ethnological\\S*",
...
SDG11_5_x = c(
  ...
  paste(
    death_ls,
    "economic loss|\\bmissing|affected|damag\\S*|\\bdisrupt\\S*",
    sep = '|'
  ),
  disaster_ls
)
...
SDG11_6_x = c(
  paste(
    "contaminat\\S*|environment impact.?|environmental impact.?|impact.? environment\\S*",
    "\\bair.?quality|\\bsmog.?|\\bhaze|pm.?2.?5|pm.?10|particulate matter.?",
    waste_ls,
    "spill\\S*|\\bsoil\\b|footprint.?|pollution\\S*|pollutant\\S*|recycl\\S*|circular|toxin|toxic",
    sep = "|"
  ),
  urban_ls
)
...
SDG11_7_x = c(
  "safe|secure|inclusi\\S*|accessible|available|reachable|\\bgreen\\b|\\bpublic|\\bnatur\\S*",
  "\\bspace.?\\b|bikeway"
)
...
SDG11_a_x = c(
  "econom\\S*|social|socio\\S*|environment\\S*|socio.?economic|ecolog\\S*",
...
SDG11_b_x = c(
  paste(urban_ls, "local government", sep = "|"),
  policy_ls,
...
SDG11_b_z <- c(
  "risk management\\S*|risk reduction.?",
  paste('climate change', disaster_ls, sep = "|")
)
...
SDG12_1_x1 = c(
  "sustainab\\S*|\\bgreen\\b|\\bgreener\\b|\\bclean\\b|Responsible|Eco.?Friendly|Environmentally.?Friendly|Environment.?Friendly|Recyclable|\\befficien\\S*",
...
SDG12_2_x = c(
  "sustainab\\S*|\\befficien\\S*",
  "resource.?|material.?\\b|\\bland\\b",
  "manag\\S*|\\busing\\b|\\buse\\b|\\buses\\b|consumption.?|production.?"
)
SDG12_2_y = c(
  "manag\\S*|conserv\\S*|protocol.?",
  paste(
    "natural.?resource.?|natural capital|natural asset.?|biological resource.?|Renewable resource.?|raw material.?",
    "basic material.?|primary material.?",
    "\\btimber.?|land resource.?|forest resource.?|mineral resource.?",
    sep = "|"
  )
)
SDG12_2_z = c("resource.?efficien\\S*|efficient resource.?",
              increase_ls)
SDG12_2_w = c(
  "material.?\\b|resource.?|\\bland\\b|biomass|fossil.?fuel.?|Fossil.?energy|metal ores|industrial mineral.?",
  ...
  paste(
    "manag\\S*|per GDP|per capita|\\befficien\\S*",
    reduce_ls,
    sep = "|"
  )
)
...
SDG12_2_x <-
  func_to_exclude_terms(which_sdg_term = SDG12_2_x,
                        terms_to_exclude = c("Human Resource.?"))
...
SDG12_3 = c(
  paste(reduce_ls, "halve|\\bhalf", sep = "|"),
...
SDG12_4_x = c(
  paste(
    "manag\\S*|\\btreat\\S*|agreement.?|responsib\\S*|collected|controlled|handled|removal|life cycle|per capita",
...
temp      <- SDG12_4_x
ex_ls <-
  "chemical.? industr\\S*|Chemical.? division.?|Chemical.? segment\\S*|chemical.? business\\S*|Chemical.? Index|treatment plant.?" %>%
  strsplit(., split = "\\|") %>% unlist()
SDG12_4_x <-
  lookaround_nearby_n(
    word_ls1 = temp[1],
...
SDG12_4_x2 = c(
  "\\bsafe\\S*|responsib\\S*|sustainab\\S*|manag\\S*",
...
SDG12_4_z = c(
  "environmental impact.?|environmental issue.?|environmental risk.?",
  paste(
    reduce_ls,
    "de.?coupl\\S*|informed",
...
SDG12_5_x = c(
  paste(
    "recycl\\S*|\\bre.?us\\S*|re.?utiliz\\S*|re.?possess\\S*|\\bregenerat\\S*|circular",
...
SDG12_6_y <-
  func_to_exclude_terms(
    which_sdg_term = SDG12_6_y,
    terms_to_exclude = c("Social Responsibility Committee")
  )
...
SDG12_a = c(
  "sustainab\\S*|\\bgreen\\b|\\bgreener\\b|\\bclean",
...
SDG12_b = c(
  "sustainab\\S*|\\bgreen\\b",
  "touris\\S*",
  "monitor\\S*|manag\\S*|accounting|\\btrack\\S*|\\bregulat\\S*|administer\\S*|govern\\S*"
)

SDG12_c = c(
  "fossil|\\bcoal\\b|\\bcoals\\b|petrol|natural gas|crude oil.?|gasoline|kerosene|non.?renewable.?|traditional energy|Conventional energy",
...
### climate_ls ----
climate_ls = paste(
  "\\bclimat\\S*|global change|Global dimming|global environment change|\\bENSO\\b|\\bEl Ni|Southern Oscillation",
...
SDG13_1 = c(
  ...
  paste(
    "resilien\\S*|adapt\\S*|adjust\\S*|\\bcombat\\S*|\\blimit.?\\b",
    reduce_ls,
    sep = '|'
  )
)
...
SDG13_2_x = paste0("^(?!.*(?:data.?base\\S*|data.?base\\S*)).*",
                   ## to exclude this
                   "(",
                   SDG13_2_x,
                   ").+")
...
SDG13_2_y = c(
  paste(
    "ocean acidification|deforest\\S*|desertificati\\S*|animal farming|farming livestock\\S*|fertilizer\\S*",
...
SDG13_2_z = c(
  paste(climate_ls, climate_good_ls, sep = "|"),
  paste(
    "adapt\\S*|Mitigation\\S*|action\\S*|\\bact\\b|legislation\\S*|contribut\\S*|convention\\S*|prepar\\S*|fight\\S*|\\bcombat\\S*|tackl\\S*",
...
SDG13_2_w1 <- c(
  emission_ls,
  paste(
    "captur\\S*|sequest\\S*|\\blimit.?\\b|\\bremov\\S*|absorb\\S*|stabiliz\\S*",
    reduce_ls,
    sep = "|"
  )
)
...
SDG13_2_w2 <-
  paste("energy transition\\S*|decarboni\\S*", renewable_ls, sep = "|")
SDG13_2_w2 <-
  paste0(
    "^(?!.*(?:Power segment\\S*|Power Green Camp\\S*|Renewables & Power|wind up|",
...
SDG13_3 = c(
  ...
  paste(
    "adapt\\S*|captur\\S*|contribut\\S*|convention\\S*",
    reduce_ls,
    policy_ls,
    sep = '|'
  ),
...
SDG13_a_x = c(
  paste(climate_ls, climate_good_ls, 'kyoto protocol', sep = '|'),
  paste(
    finance_ls,
    "dollar.?\\b|capital.?|mobiliz\\S*|convention\\S*",
    sep = '|'
  )
)
...
SDG14_4_x = c(
  "\\bfish\\S*|seafood|blue food|\\bcatch\\b|\\bcatches\\b|bycatch|harvest\\S*|overfish\\S*",
  paste(
    illegal_ls,
...
SDG14_6_x = c(
  "\\bfish\\S*|seafood|\\bcatch\\b|\\bcatches\\b|bycatch|harvest\\S*|aquaculture|blue food|mariculture",
...
SDG14_6_y = c(
  ...
  paste(
    illegal_ls,
    "unreported|unregulated|destructive|destro\\S*|diminish\\S*|exploit\\S*|poach\\S*|traffick\\S*",
    sep = "|"
  ),
  paste(
    "instrument\\S*|\\blaw\\S*|enforc\\S*|restrict\\S*|prohibit\\S*|\\bquota\\b|\\bregulat\\S*|monitor\\S*",
    policy_ls,
    sep = '|'
  )
)
...
temp <-
  "scient\\S*|\\bknowledge|research\\S*|technolog\\S*|budget.?|\\bspending|health\\S*"
...
SDG14_b = c(
  paste("artisan\\S*|tradition\\S*", "small.?scale", sep = '|'),
...
SDG15_1_x = c(
  "terrestrial|inland|fresh.?water|water eco.?system.?|\\bforest\\S*|wood.?land|wetland|marsh|mountain.?|dryland|rain.?forest\\S*|agroforest\\S*|tundra",
...
SDG15_2_x = c(
  "\\btree.?plant\\S*|plant.? tree\\S*|planting tree\\S*|deforest\\S*|afforest\\S*|reforest\\S*|silvicultur\\S*|mangrove\\S*|silviculture|shrub",
  paste(
    "conserv\\S*|\\brestor\\S*|sustain\\S*|manag\\S*|mitig\\S*|health\\S*|resilien\\S*",
    "stewardship\\S*|certifi\\S*|audit\\S*|account\\S*",
    increase_ls,
    sep = "|"
  )
)
...
SDG15_2_y <-
  c(
    "deforestation|forest product\\S*|timber\\S*|degraded forest\\S*",
...
SDG15_2_y <-
  lookaround_nearby_n(word_ls1 = temp[1],
                      ...
                      n = 9)
SDG15_2_z <-
  "Preventive silviculture|\\bREDD\\b|R\\.E\\.D\\.D\\.|Reducing Emissions from Deforestation and forest Degradation"
...
SDG15_3 = c(
  "degrad\\S*|desertif\\S*|contamin\\S*|pollut\\S*|de.?nitrificat\\S*|health\\S*|nitro\\S*|drought\\S*|flood\\S*",
  "\\bland|\\bsoil\\b|sediment\\S*",
  paste(
    "\\bcombat\\S*|conserv\\S*|\\brestor\\S*|sustain\\S*|manag\\S*",
    reduce_ls,
    sep = "|"
  )
)
...
SDG15_5_x <-
  func_to_exclude_terms(which_sdg_term = SDG15_5_x, terms_to_exclude = "natural gas")

SDG15_5_y = paste(
  "Protected Area\\S*|protection area\\S*|conservation area\\S*|conservation park\\S*",
  "biodivers\\S*|biological diversity|Red List|habitat conversion|habitat degradation\\S*|habitat fragmentation\\S*|habitat loss",
  sep = "|"
)
SDG15_5_z = c(
  paste(
    "habitat|\\breserve.?\\b|sanctuar\\S*|threaten\\S*|endanger\\S*|extinct\\S*|Refuge\\b|\\bcorridor.?",
...
SDG15_5_z <-
  lookaround_nearby_n(
    ...
    word_ls2 = temp[2],
    n = 4,
    exclude = c("habitat for human", "human habitat")
  )


SDG15_6 = c(
  "genetic resource\\S*|genetic|\\bgene\\b",
  "\\bfair\\S*|equitab\\S*|access\\S*|availab\\S*|equal\\S*|\\bshare.?\\b|sharing"
)

SDG15_7 = c(
  paste("poach\\S*|traffic\\S*", illegal_ls, sep = "|"),
...
## ------------------------------------------------------------------------------------ ##
SDG15_9 = c(
  paste("ecosystem\\S*", "biodiver\\S*", sep = '|'),
  "\\bnation\\S*|countr\\S*|region\\S*|local",
  paste(
    "development|poverty reduction|accounting|reporting|Aichi|action plan\\S*|target\\S*|planning|strateg\\S*",
    policy_ls,
    sep = "|"
  )
)
...
SDG15_a = c(
  paste(ecosystem_ls, "biodiver\\S*", sep = '|'),
  paste(
    finance_ls,
    "revenue\\S*|\\bvalue|incentiv\\S*|\\bspending\\S*",
    sep = '|'
  ),
...
SDG15_a <-
  gsub(
    "\\.\\+",
    "(?!.*business environment)(?!.*political environment)(?!.*social environment)",
    SDG15_a
  ) ## to exclude either of the three terms
...
SDG15_c = c(
  "poach\\S*|traffic\\S*|illicit\\S*|illegal",
  "species|flora|fauna|wildlife|logg\\S*|animal\\S*|mammal\\S*|\\bhunt\\S*|cultivat\\S*|\\bcatch\\b|\\bcatches\\b",
...
SDG15_general_x = paste(
  func_AND_vector(c(ecosystem_ls, "resilien\\S*")),
...
SDG15_general_y = c(
  ecosystem_ls,
  paste(
    "impact.?\\b|conserv\\S*|\\brestor\\S*|sustaina\\S*|manag\\S*|mitig\\S*",
...
SDG16_1_x = c(
  "violen\\S*|victim\\S*|homicid\\S*|murder\\S*|\\bkill\\S*|assault\\S*|assassination\\S*",
  paste(
    reduce_ls,
...
SDG16_1_y = c(
  "verbal|physical|domestic|psychological|child|\\bsex\\S*",
...
SDG16_3_x = c(
  "access\\S*|resort to|promot\\S*",
  paste(
    "\\bcode.?\\b|\\bnorm.?\\b|justice|authorit\\S*|\\bpolice\\S*|prosecutor.?|\\brule.?\\b",
...
SDG16_3_y <-
  gsub("\\.\\+", "(?!.*Annual Report)", SDG16_3_y) ## to exclude
...
SDG16_4_x <-
  c(
    paste(
      illegal_ls,
      "crime|cybercrime|irregular\\S*|prohibited|smuggl\\S*|unwarranted",
      "stolen|\\bsteal|theft|seized|traced|conflict\\S*|traffic\\S*|terroris\\S*",
      sep = "|"
    ),
    paste(
      finance_ls,
      "\\barm\\S*|\\bgun\\S*|weapon\\S*|ammunition\\S*|rifle\\S*|pistol\\S*|firearm\\S*|shotgun\\S*|\\basset\\S*",
      sep = '|'
    ),
    paste(
      "\\brecovery|\\breturn|\\bagainst\\b|\\bAnti.?\\b|\\bcombat\\S*",
      reduce_ls,
      sep = "|"
    )
  )
...
SDG16_4_y <-
  "unauthorized acquisition\\S*|money.?launder|money.?washing|launder money|laundering money"



SDG16_5_x = c(
  paste(
    "corrupt\\S*|\\bbrib\\S*|lobbying|extortion\\S*|cronyism|nepotism|parochialism|patronage\\S*",
    "influence peddling|\\bgraft\\S*|embezzl\\S*|fraud\\S*|\\bextort\\S*|blackmail|Favoritism|clientelism",
    "Tax evasion\\S*",
    sep = "|"
  ),
  paste(
    "identif\\S*|takes into account|aware of|\\bpolicy|\\bpolicies|legislation\\S*|\\bregulat\\S*",
    "manag\\S*|\\brule.?\\b",
...
SDG16_5_y = c(
  "contact\\S*|asked|\\bpay\\b|\\bpaid|extort\\S*|\\bentrust\\S*",
  "\\bofficial.?"
)
...
SDG16_6_x = c(
  paste(
    "institut\\S*|\\bgovern\\S*|legislat\\S*|judicia\\S*",
  ...
  paste(
    "effective|accountab\\S*|transparent\\S*|satisf\\S*|availability|accessib\\S*|Waiting time",
...
SDG16_6_z2 <-
  c(
    "public financ\\S*|Public Expenditure.?|government budget\\S*|government expenditure.?",
    "management.?|Accountab\\S*"
  )
SDG16_6_z3 <-
  c(
    "Actual expenditure.?|Aggregate expenditure.?|final budget\\S*",
...
temp      <-
  "responsive|inclusive|participatory|representative.?|independen\\S*"
...
SDG16_7_w2 = c(
  "\\bBoard\\b|Committee\\b|leadership",
  "inclusive|participatory|representative.?",
  paste(
    "\\bgender|\\bsex\\S*|ethni\\S*|\\brace|racial|\\breligion",
    disability_ls,
    'disadvantaged',
    female_ls,
    LGBT_ls,
...
SDG16_8_x = c(
  "institution\\S*|global govern\\S*|international organization\\S*",
...
SDG16_10_x = c(
  "public access|availab\\S*|attack\\S*|freedom|\\bkill\\S*|kidnap\\S*|murder\\S*|enforc\\S*|disappear\\S*|deten\\S*|detain\\S*|tortur\\S*",
...
temp <-
  paste("\\bagainst\\b|\\bAnti.?\\b|\\bcombat\\S*", reduce_ls, sep = "|")
SDG16_a_x1 = c(
  "\\bnation\\S*|countr\\S*|\\bstate.?\\b|domestic|international",
...
SDG16_a_y1 = c(
  "\\bnation\\S*|countr\\S*|\\bstate.?\\b|domestic",
  "independen\\S*",
...
SDG16_b = c(
  "\\blaw\\S*|legislat\\S*|\\bpolicy|\\bpolicies|human right\\S*|protect\\S*|safeguard\\S*",
...
SDG16_general = paste("ethnic conflict\\S*|exonerat\\S*|justice system\\S*",
                      func_AND_plus(
                        c(
                          "environment\\S*",
                          "\\blaw\\S*|govern\\S*|\\benact\\S*|legistlat\\S*"
                        )
                      ),
                      sep = "|")
...
SDG16_general_x3 <-
  c(
    "\\bdata|\\binformation|\\bcyber\\b|\\bcloud\\b|\\bInternet\\b|\\bnetwork\\b|\\bmobile\\b",
    paste(
      "privacy|security|protection|sensitive|confidential\\S*|\\battack\\S*",
...
SDG17_1 = c(
  "domestic resource\\S*|domestic capacity|domestic budget\\S*",
  "revenue\\S*|budget\\S*|\\btax|expenditure\\S*"
)
SDG17_1 <- func_AND_plus(SDG17_1)
SDG17_1 <-
  gsub("\\.\\+", "(?!.*governmental charge)", SDG17_1) ## to exclude
